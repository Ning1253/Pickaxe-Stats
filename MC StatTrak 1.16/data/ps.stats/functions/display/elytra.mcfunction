...
data modify storage ps.stats:item Item set from entity @s Inventory[{Slot:102b}]

# Clearing items in the shulker box, copying stone with lore from elytra_loot into shulker.
data modify block 0 0 0 Items set value []
loot insert 0 0 0 loot ps.stats:elytra_loot
...
execute as @a at @s if block 0 0 0 yellow_shulker_box{Items:[{tag:{StatTrak:"true"}}]} run loot replace entity @s armor.chest 1 mine 0 0 0 minecraft:air{drop_contents:1b}
