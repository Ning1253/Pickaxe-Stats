scoreboard players set @s ts.ore 0
scoreboard players operation @s ts.ore += @s ts.coal
scoreboard players operation @s ts.ore += @s ts.iron
scoreboard players operation @s ts.ore += @s ts.redstone
scoreboard players operation @s ts.ore += @s ts.lapis
scoreboard players operation @s ts.ore += @s ts.gold
scoreboard players operation @s ts.ore += @s ts.diamond
scoreboard players operation @s ts.ore += @s ts.emerald
scoreboard players operation @s ts.ore += @s ts.quartz


scoreboard players set @s ts.wood 0
scoreboard players operation @s ts.wood += @s ts.oak
scoreboard players operation @s ts.wood += @s ts.spruce
scoreboard players operation @s ts.wood += @s ts.birch
scoreboard players operation @s ts.wood += @s ts.jungle
scoreboard players operation @s ts.wood += @s ts.acacia
scoreboard players operation @s ts.wood += @s ts.dark_oak

scoreboard players set @s ts.totalsnow 0
scoreboard players operation @s ts.totalsnow += @s ts.snow
scoreboard players operation @s ts.totalsnow += @s ts.snow_block

scoreboard players set @s ts.totaldug 0
scoreboard players operation @s ts.totaldug += @s ts.dirt
scoreboard players operation @s ts.totaldug += @s ts.grass
scoreboard players operation @s ts.totaldug += @s ts.sand
scoreboard players operation @s ts.totaldug += @s ts.gravel
scoreboard players operation @s ts.totaldug += @s ts.clay
scoreboard players operation @s ts.totaldug += @s ts.totalsnow
scoreboard players operation @s ts.totaldug += @s ts.soulsand

scoreboard players set @s ts.hits 0
scoreboard players operation @s ts.hits += @s ts.ws_hits
scoreboard players operation @s ts.hits += @s ts.ss_hits
scoreboard players operation @s ts.hits += @s ts.is_hits
scoreboard players operation @s ts.hits += @s ts.gs_hits
scoreboard players operation @s ts.hits += @s ts.ds_hits
scoreboard players operation @s ts.hits += @s ts.ns_hits